...
Default operating model for substantial coding work: **the opencode main thread is the orchestrator and validator; subagents do the bulk implementation, exploration, and adversarial review.**
...
- Dispatch to `worker` subagents (via the `task` tool with `subagent_type: "general"`) for implementation (they follow `/implement`: `/tdd` at pre-agreed seams). Respect blocking edges: only dispatch tickets whose blockers are done; parallelize the frontier when edges allow.
...
- Answer from your own judgment as orchestrator; spawn another `explore` or `general` subagent (via the `task` tool) when a question needs runnable evidence you don't already have, then answer.
